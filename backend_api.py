...
def sync_user_tickets(user_id: int = Depends(get_current_user_id)):
    """Synchronizuj user tikety - checkni finished zápasy z API-Football"""
    try:
        saved_tickets = repo.get_saved_tickets(user_id)
        
        # Pro každý pending tiket - checkni zápasy
        for ticket_data in saved_tickets:
            ...
            # Pro každý zápas v tiketu
            for selection in ticket_obj.selections:
                try:
                    # Stáhni výsledek z API-Football
                    ...
                    url = f"https://v3.football.api-sports.io/fixtures?id={selection.match_id}"
                    ...
                    if status != "FT":  # Finished
                        all_settled = False
                        continue
                    
                    # Zápas je skončený - vyhodnoť
                    score = fixture.get("score", {})
                    home = score.get("fulltime", {}).get("home") or 0
                    away = score.get("fulltime", {}).get("away") or 0
                    ...
                    if selection.market_type == MarketType.MATCH_WINNER:
                        ...
                        try:
                            ...
                            result = "won" if total > threshold else "lost"
                        except:
                            pass
                    ...
                    # Ulož result
                    if result:
                        db.update_selection_result(selection.id, result)
                        if result == "lost":
                            all_won = False
            ...
            # Pokud jsou všechny zápasy settled - updatuj ticket status
            if all_settled and len(ticket_obj.selections) > 0:
                final_status = "won" if all_won else "lost"
                db.update_ticket_status(ticket_data["ticket_id"], final_status)
        
        # Vrať všechny tikety (s updated statusy)
        saved_tickets = repo.get_saved_tickets(user_id)
        result = []
        for row in saved_tickets:
            created_at_str = None
