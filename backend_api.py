...
def list_saved_tickets(user_id: int = Depends(get_current_user_id)):
    ...
    for row in repo.get_saved_tickets(user_id):
        if row["status"] == "pending":
            # Zkontrolovat - má tento tiket JIŽ výsledky v DB?
            selections = row.get("selections", [])
            all_have_results = all(s.get("result") and s.get("result") != "pending" for s in selections)
            
            # Pokud VŠECHNY selections mají výsledek v DB - NEVOLÁME API!
            if all_have_results:
                # Všechny nohy mají result - spočítej status tiketu z nich
                legs = [s.get("result") == "won" for s in selections]
                if any(l is False for l in legs):
                    repo.set_ticket_status(row["ticket_id"], "lost")
                elif all(l is True for l in legs):
                    repo.set_ticket_status(row["ticket_id"], "won")
            else:
                # JENOM POKUD nemáme všechny výsledky - taháme z API
                selection_ids = [s.get("id") for s in selections] if isinstance(selections, list) else []
...
def _try_settle_ticket(provider, ticket: Ticket, selection_ids: list[int] = None) -> Optional[str]:
    ...
    for i, selection in enumerate(ticket.selections):
        try:
            ...
            # Ulož "pending" i když API selhalo
        ...
        if not result["is_finished"] or result["home_goals"] is None:
            leg_results.append(None)
            # Ulož "pending" když zápas neskončil
            if selection_ids and i < len(selection_ids):
                db.update_selection_result(selection_ids[i], "pending")
            continue
