...
def settle_tickets(user_id: int = Depends(get_current_user_id)):
    """
    Projde všechny pending tikety uživatele a zkusí je vyhodnotit podle
    výsledků zápasů z API-Football. Volá se automaticky při otevření
    historie, nebo může být voláno cron jobem.
    """
    provider = data_provider.get_provider(Sport.FOOTBALL)
    ...
    for row in pending_rows:
        ...
        for sel in selections:
            ...
            if not match_id or match_id == 0:
                ...
                continue
            try:
                result = provider.get_fixture_result(str(match_id))
                if result is None:
                    all_finished = False
                    continue
                sel_won = _evaluate_selection(sel, result)
                if sel_won is None:
                    all_finished = False
                elif sel_won:
                    pass  # výběr vyhrál
        ...
        if all_finished:
            ...
            if row.get("actual_stake_amount"):
                db.update_ticket_profit_loss(ticket_id, row["actual_stake_amount"], row.get("actual_odds") or row["total_odds"], new_status)
            settled += 1
