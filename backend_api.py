                ...
                try:
                    if sel.result == "pending":
                        ...
                        if fixture_result and fixture_result.get("is_finished") and fixture_result.get("home_goals") is not None:
                            home_g, away_g = fixture_result["home_goals"], fixture_result["away_goals"]
                            outcome = evaluate_selection_outcome(sel, home_g, away_g)
                            return (sel, home_g, away_g, "won" if outcome is True else "lost" if outcome is False else "pending")
                    elif sel.result != "pending" and sel.home_goals is None:
                        match_result = provider.get_fixture_result(sel.match_id)
                        fixture_result = data_provider.adapt_fixture_result(match_result)
                        if fixture_result and fixture_result.get("home_goals") is not None:
                            return (sel, fixture_result["home_goals"], fixture_result["away_goals"], sel.result)
                except:
                    pass
                return None
            
            with ThreadPoolExecutor(max_workers=5) as executor:
                for future in as_completed([executor.submit(fetch_scores, sel) for sel in tr.selections]):
                    result = future.result()
                    if result:
                        sel, home_g, away_g, new_result = result
                        sel.home_goals = home_g
                        sel.away_goals = away_g
                        sel.result = new_result
