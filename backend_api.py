...
@app.post("/admin/_debug-preview-today")
def admin_debug_preview_today(request: Request):
    ...
    if result["safe"] is None:
        ...
        if wider_result["safe"] is not None:
            result = wider_result
            used_wider = True
            horizon_note = f"V okně {time_frame_days} den appka nenašla dost kandidátů, nabízí zápasy až za {wider_days} dny."
        else:
