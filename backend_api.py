...
def _fetch_candidate_matches(sports: list[Sport], time_frame_days: int,
    ...
    # Vypočítej hranice dnů co se mají brát
    today = date.today()
    
    # PRIORITA: pokud je custom_date, vezmi JEN ten den!
    if custom_date:
        try:
            target_date = date.fromisoformat(custom_date)
            min_date = target_date
            max_date = target_date + timedelta(days=1)  # až do konce toho dne
        except ValueError:
            min_date = today
            max_date = today + timedelta(days=time_frame_days)
    # Pokud jsou date_from/date_to, vezmi ten rozsah
    elif date_from and date_to:
        try:
            min_date = date.fromisoformat(date_from)
            max_date = date.fromisoformat(date_to) + timedelta(days=1)
        except ValueError:
            min_date = today
            max_date = today + timedelta(days=time_frame_days)
    # Jinak vezmi podle time_frame_days
    else:
        min_date = today
        max_date = today + timedelta(days=time_frame_days)
    ...
    for sport in sports:
        ...
        # FILTR: vezmi jen zápasy v rozsahu dnů!
        filtered_items = []
        for item in raw_items:
            # Hledáme datum v item - různé formáty podle sportu
            item_date_str = None
            if isinstance(item, dict):
                item_date_str = item.get("fixture", {}).get("date") or item.get("date") or item.get("kickoff")
            
            if item_date_str:
                try:
                    # Vezmi jen datum část (YYYY-MM-DD)
                    item_date = date.fromisoformat(item_date_str[:10])
                    # Vezmi jen pokud je v rozsahu
                    if min_date <= item_date < max_date:
                        filtered_items.append(item)
                except (ValueError, TypeError):
                    # Pokud se nepodaří parsovat datum, vezmi to (fallback)
                    filtered_items.append(item)
            else:
                # Pokud nema datum, vezmi to
                filtered_items.append(item)
        
        sport_matches = builders[sport](provider, filtered_items)
