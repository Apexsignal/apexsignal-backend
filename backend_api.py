...
async def evaluate_selection_from_api(selection) -> Optional[str]:
    """
...
# ============ AUTO-SETTLE ENDPOINTS ============

@app.get("/admin/auto-settle-now")
async def trigger_auto_settle_now():
    """Ručně spusť auto-settle (testing only)"""
    try:
        await auto_settle_tickets()
        return {"status": "auto-settle completed"}
    except Exception as e:
        logger.error(f"❌ Auto-settle error: {e}")
        return {"error": str(e)}
...
async def sync_user_tickets(user_id: int = Depends(get_current_user_id)):
    """Synchronizuj user tikety - checkni finished zápasy a aktualizuj status"""
    try:
        await auto_settle_tickets()
        # Vrať updated tikety
        saved_tickets = repo.get_saved_tickets(user_id)
        ...
        for row in saved_tickets:
            ...
            result.append(tr)
        return {"tickets": result, "synced": True}
    except Exception as e:
        logger.error(f"❌ Sync error: {e}")
        return {"error": str(e), "synced": False}


# Když server startuje, spusť auto-settle v background každých 5 minut
async def background_auto_settle():
    """Background task - spouští auto-settle každých 5 minut"""
    await asyncio.sleep(30)  # Počkej 30 sekund po startu
    while True:
        try:
            await auto_settle_tickets()
        except Exception as e:
            logger.error(f"❌ Background auto-settle error: {e}")
        
        await asyncio.sleep(300)  # Čekej 5 minut


@app.on_event("startup")
async def start_background_tasks():
    """Spusť background tasks když se server startuje"""
    asyncio.create_task(background_auto_settle())
