...
def save_ticket(req: SaveTicketRequest, user_id: int = Depends(get_current_user_id)):
    ...
    try:
        ...
        print(f"[save_ticket] Warning - couldn't settle ticket on save: {e}")
...
def list_saved_tickets(user_id: int = Depends(get_current_user_id)):
    ...
    """
    provider = data_provider.get_provider(Sport.FOOTBALL)
    print(f"[list_saved_tickets] START - będę sprawdzać pending tikets")
    pending_count = 0
    for row in repo.get_saved_tickets(user_id):
        if row["status"] == "pending":
            ...
            selection_ids = [s.get("id") for s in row.get("selections", [])]
            print(f"[list_saved_tickets] Ticket {ticket_id}: pending, {len(selection_ids)} selections, IDs: {selection_ids}")
            new_status = _try_settle_ticket(provider, row["ticket"], selection_ids)
            if new_status is not None:
                print(f"[list_saved_tickets] Ticket {ticket_id}: status changed to {new_status}")
                repo.set_ticket_status(ticket_id, new_status)
                repo.set_live_alert(ticket_id, None)
    print(f"[list_saved_tickets] Processed {pending_count} pending tickets")
...
def _try_settle_ticket(provider, ticket: Ticket, selection_ids: list[int] = None) -> Optional[str]:
    ...
    """
    print(f"[_try_settle_ticket] START - selections: {len(ticket.selections)}, selection_ids: {selection_ids}")
    ...
    if any(r is False for r in leg_results):
        print(f"[_try_settle_ticket] RETURN: lost")
        return "lost"
    if leg_results and all(r is True for r in leg_results):
        print(f"[_try_settle_ticket] RETURN: won")
        return "won"
    print(f"[_try_settle_ticket] RETURN: None (pending)")
