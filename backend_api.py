...
def get_current_user_id(authorization: str = Header(default="")) -> int:
    ...
    z podepsaného tokenu v Authorization headeru, nikdy z user_id, co by
    klient mohl poslat sám v těle požadavku. Bez tohohle by si kdokoli
    mohl jen tak "být" jiný uživatel a vidět jeho tikety.
    """
    if not authorization.startswith("Bearer "):
        raise HTTPException(status_code=401, detail="Chybí přihlašovací token")
    user_id = auth.verify_token(authorization[len("Bearer "):])
    if user_id is None:
...
class Repo:
    ...
    def get_real_results_report(self, user_id: int) -> dict:
        """
        Souhrn SKUTEČNĚ vsazených tiketů PRO JEDNOHO uživatele — ne všech
        uložených, jen těch, kde uživatel zadal vlastní kurz a částku
        (viz set_actual_stake). Tiket, co si jen uložíš na rozmyšlenou
        a nikdy nevsadíš, se sem nepočítá — appka neřeší, jestli se
        reálně vsadilo, jen to, co jí uživatel sám potvrdí.
        """
        ...
        # Časová řada pro graf na frontendu — appka bere pořadí podle
        # ticket_id (roste s časem uložení), ne přesný timestamp, ale to
        # pro kumulativní graf zisku/ztráty stačí.
        cumulative = 0.0
        history = []
...
def _build_football_matches(provider, raw_fixtures: list[dict]) -> list[MatchInput]:
    ...
    # Tabulku soutěže appka stahuje jen JEDNOU za běh per liga, ne pro
    # každý zápas znovu — šetří API dotazy, když appka generuje tikety
    # z víc zápasů stejné soutěže najednou.
    ...
    for raw in raw_fixtures:
        ...
        # "Dead rubber" heuristika z tabulky soutěže (viz adapt_standings_for_motivation).
        ...
        if league_id:
            try:
                if league_id not in standings_cache:
                    standings_cache[league_id] = provider.get_standings(league_id, fixture.get("season"))
                standings = standings_cache[league_id]
                home_dead_rubber = data_provider.adapt_standings_for_motivation(standings, fixture["home_team"])
                away_dead_rubber = data_provider.adapt_standings_for_motivation(standings, fixture["away_team"])
                data_availability["standings_motivation"] = bool(standings)
            except Exception:
                pass
        ...
        matches.append(data_provider.normalize_to_match_input(
