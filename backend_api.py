    ...
    for row in saved_rows:
        ...
        for i, sel in enumerate(tr.selections):
            if i < len(raw_sels):
                ...
                # FALLBACK: Pokud je tiket won/lost ale selection ještě pending, znamená to že se nehodnotil
                # Stejně to ale zkusíme — backend ho pak vyhodnotí
                if sel.result == "pending" and row["status"] in ("won", "lost"):
                    sel.result = row["status"]  # Dočasně nastav na status tiketu
...
def _try_settle_ticket(provider, ticket: Ticket, selection_ids: list[int] = None) -> Optional[str]:
    ...
    for i, selection in enumerate(ticket.selections):
        ...
        # Ulož výsledek jednotlivého zápasu v DB
        if selection_ids and i < len(selection_ids) and outcome is not None:
