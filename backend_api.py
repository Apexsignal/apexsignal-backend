...
from probability_model import (
    ...
    _passes_edge_tolerance, NEAR_MISS_TOLERANCE_STEPS,
)
...
def _debug_generate_today(request: Request, email: str, risk_level: int = 20):
    ...
    pro dnešek na zadaný účet a rovnou uloží+pošle na Telegram, pokud
    najde platnou kombinaci. Po použití appka tenhle endpoint zase smaže."""
    admin_key_expected = os.environ.get("ADMIN_TASK_KEY")
    if not admin_key_expected or request.headers.get("X-Admin-Key") != admin_key_expected:
        raise HTTPException(status_code=403, detail="Neplatný nebo chybějící X-Admin-Key")
    ...
    return {
        ...
        "selections": [{"match": f"{s.home_team} - {s.away_team}", "market": s.market_type.value, "selection": s.selection, "reasoning": s.reasoning} for s in ticket.selections],
    }
