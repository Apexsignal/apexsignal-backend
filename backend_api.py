    ...
    with ThreadPoolExecutor(max_workers=FIXTURE_ENRICHMENT_WORKERS) as executor:
        ...
        for future in as_completed(future_to_idx):
            ...
            try:
                ...
                print(f"[tickets/generate] hotovo {len(matches)}/{len(raw_fixtures)} (poslední dokončené pořadí: {idx})")
            except Exception as exc:
                print(f"[tickets/generate] zápas {idx} selhal, appka ho přeskočí: {exc}")
...
def list_saved_tickets(user_id: int = Depends(get_current_user_id)):
    ...
    for row in repo.get_saved_tickets(user_id):
        if row["status"] == "pending":
            selection_ids = [s.get("id") for s in row.get("selections", [])] if isinstance(row.get("selections"), list) else []
    ...
    for row in saved_rows:
        ...
        for i, sel in enumerate(tr.selections):
            if i < len(raw_sels):
                raw = raw_sels[i]
                sel.result = raw.get("result") or "pending"
                sel.id = raw.get("id")
                # Přepiš odds z DB (může být upraven ručně)
                if raw.get("odds"):
                    sel.odds = float(raw["odds"])
                
                # Fallback: pokud je selection pending ale tiket je done, zkus stáhnout skóre
                # POKUD JE ZÁPAS SKONČENÝ - vyhodnoť ho
                if sel.result == "pending" and row["status"] in ("won", "lost"):
                    try:
                        match_result = provider.get_fixture_result(sel.match_id)
                        fixture_result = data_provider.adapt_fixture_result(match_result)
                        if fixture_result and fixture_result.get("is_finished") and fixture_result.get("home_goals") is not None:
                            # Zápas se skončil - spočítej výsledek
                            home_g = fixture_result["home_goals"]
                            away_g = fixture_result["away_goals"]
                            sel.home_goals = home_g
                            sel.away_goals = away_g
                            
                            # Spočítej outcome
                            from probability_model import evaluate_selection_outcome
                            outcome = evaluate_selection_outcome(sel.raw, home_g, away_g)
                            sel.result = "won" if outcome is True else "lost" if outcome is False else "pending"
                            print(f"[fallback] Match {sel.match_id}: SPOČÍTÁNO - {sel.result}")
                    except Exception as e:
                        pass  # fallback failed, zůstane pending
                
                # Pokud je tiket vyhodnocený (won/lost), stáhni skóre z API
                if sel.result != "pending":
                    try:
                        match_result = provider.get_fixture_result(sel.match_id)
                        fixture_result = data_provider.adapt_fixture_result(match_result)
                        if fixture_result and fixture_result.get("home_goals") is not None:
                            sel.home_goals = fixture_result["home_goals"]
                            sel.away_goals = fixture_result["away_goals"]
                            print(f"[list_saved_tickets] Match {sel.match_id}: {sel.home_goals}-{sel.away_goals}")
                        else:
                            print(f"[list_saved_tickets] Match {sel.match_id}: NO GOALS")
                    except Exception as e:
                        print(f"[list_saved_tickets] Match {sel.match_id}: ERROR - {e}")
                        pass
        ...
        if tr.selections:
            print(f"[list_saved_tickets-FINAL] Ticket {tr.ticket_id}: status={tr.status}")
            for sel in tr.selections[:2]:  # První 2 selections
                print(f"  - {sel.home_team} vs {sel.away_team}: result={sel.result}, goals={sel.home_goals}-{sel.away_goals}")
        
        result_list.append(tr)
    
    print(f"[list_saved_tickets] VRACIM {len(result_list)} tiketů")
    return result_list
...
def _try_settle_ticket(provider, ticket: Ticket, selection_ids: list[int] = None) -> Optional[str]:
    ...
    for i, selection in enumerate(ticket.selections):
        ...
        # Ulož výsledek jednotlivého zápasu v DB — VŽDY, i když je pending
...
def delete_selection(ticket_id: int, selection_index: int, user_id: int = Depends(get_current_user_id)):
    ...
    # Smaž selection z DB
    db.delete_selection(ticket_id, selection_index)
...
def settle_tickets(user_id: int = Depends(get_current_user_id)):
    ...
    for row in pending_rows:
        ...
        for sel in selections:
            ...
            try:
                ...
                if sel_id:
                    ...
                    print(f"[settle] Výběr {sel_id} ({sel.get('home_team','?')} vs {sel.get('away_team','?')}): {sel_result}")
                else:
                    print(f"[settle] Výběr bez id — nelze uložit výsledek. sel keys: {list(sel.keys())}")

            except Exception as e:
                print(f"[settle] Chyba při vyhodnocení zápasu {match_id}: {e}")
                all_finished = False
        ...
        if all_finished:
            ...
            settled += 1
            print(f"[settle] Tiket {ticket_id} vyhodnocen jako {new_status}")
...
{{"selections":[{{"match":"Tým A vs Tým B","pick":"co je vsazeno","verdict":"doporučuji nebo riziko nebo vynechat","risk":"nízké nebo střední nebo vysoké","reasoning":"2-3 věty proč s konkrétními fakty z webu"}}],"overall":"Celkové shrnutí tiketu 2-3 větami."}}"""}],
        ...
        try:
            ...
            detail = str(exc)
        print(f"[analyze] Anthropic API chyba: {detail}")
        raise HTTPException(status_code=502, detail=f"Analýza selhala: {detail}")
    except Exception as exc:
        print(f"[analyze] Neočekávaná chyba: {exc}")
