...
class TicketGenerateRequest(BaseModel):
    ...
    time_frame_days: int = Field(ge=1, le=14)
    custom_date: Optional[str] = None    # YYYY-MM-DD — jeden konkrétní den
    date_from: Optional[str] = None      # YYYY-MM-DD — začátek rozsahu
    date_to: Optional[str] = None        # YYYY-MM-DD — konec rozsahu
...
class TicketResponse(BaseModel):
    ...
    created_at: Optional[str] = None  # ISO 8601 timestamp — kdy byl tiket vsazen
...
def _fetch_candidate_matches(sports: list[Sport], time_frame_days: int,
                              custom_date: Optional[str] = None,
                              date_from: Optional[str] = None,
                              date_to: Optional[str] = None) -> list[MatchInput]:
    builders = {
    ...
    for sport in sports:
        ...
        raw_items = provider.get_upcoming_matches(
            sport, time_frame_days,
...
def generate_tickets(req: TicketGenerateRequest, user_id: int = Depends(get_current_user_id)):
    matches = _fetch_candidate_matches(req.sports, req.time_frame_days, getattr(req, "custom_date", None), getattr(req, "date_from", None), getattr(req, "date_to", None))
...
def list_saved_tickets(user_id: int = Depends(get_current_user_id)):
    ...
    for row in repo.get_saved_tickets(user_id):
        if row["status"] == "pending":
            settle_result = _try_settle_ticket(provider, row["ticket"])
            if settle_result is not None:
                new_status = settle_result["status"]
                leg_results = settle_result["leg_results"]
                repo.set_ticket_status(row["ticket_id"], new_status)
                repo.set_live_alert(row["ticket_id"], None)
                
                # Uloží výsledky jednotlivých zápasů
                raw_sels = row.get("selections", [])
                for i, sel_result in enumerate(leg_results):
                    if i < len(raw_sels) and sel_result is not None:
                        sel_id = raw_sels[i].get("id")
                        result_str = "won" if sel_result is True else "lost" if sel_result is False else "pending"
                        if sel_id:
                            db.update_selection_result(sel_id, result_str)
    ...
    for row in saved_rows:
        ...
        tr = TicketResponse.from_domain(
            ...
            created_at=created_at_str  # String, ne datetime!
        )
        
        # Přidej results z DB k selections
        raw_sels = row.get("selections", [])
        updated_selections = []
        for i, sel in enumerate(tr.selections):
            if i < len(raw_sels):
                raw = raw_sels[i]
                result_str = raw.get("result", "pending")
                odds_val = float(raw.get("odds", sel.odds)) if raw.get("odds") else sel.odds
                sel_id = raw.get("id")
            else:
                result_str = "pending"
                odds_val = sel.odds
            ...
            # Vytvoř nový SelectionResponse s correctním result
            updated_sel = SelectionResponse(
                ...
                result=result_str,  # ← TADY JE .result!
                id=sel_id,
            )
            updated_selections.append(updated_sel)
        
        # Vytvoř nový TicketResponse s updated selections
        tr = tr.copy(update={"selections": updated_selections})
...
def _try_settle_ticket(provider, ticket: Ticket) -> Optional[dict]:
    """
    Zkusí vyhodnotit JEDEN tiket podle aktuálních/finálních výsledků
    zápasů — vrací dict {"status": "won"/"lost", "leg_results": [...]}
    nebo None, pokud zůstává nejasný.
    """
    ...
    if any(r is False for r in leg_results):
        return {"status": "lost", "leg_results": leg_results}
    if leg_results and all(r is True for r in leg_results):
        return {"status": "won", "leg_results": leg_results}
    return None
...
def settle_tickets():
    ...
    for ticket_id, ticket in repo.get_pending_tickets():
        settle_result = _try_settle_ticket(provider, ticket)
        if settle_result is not None:
            new_status = settle_result["status"]
