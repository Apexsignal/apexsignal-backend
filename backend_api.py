...
def list_saved_tickets(user_id: int = Depends(get_current_user_id)):
    ...
    for row in saved_rows:
        # Příprava selections s results z DB
        raw_sels = row.get("selections", [])
        selections_with_results = []
        
        for i, sel in enumerate(row["ticket"].selections):
            raw = raw_sels[i] if i < len(raw_sels) else {}
            result_str = raw.get("result", "pending")
            
            # Vytvoř SelectionResponse S resultem
            sr = SelectionResponse.from_domain(sel, result=result_str)
            
            # Updatuj id a odds z DB pomocí model_copy
            updates = {}
            if raw.get("id"):
                updates["id"] = raw["id"]
            if raw.get("odds"):
                updates["odds"] = float(raw["odds"])
            
            if updates:
                sr = sr.model_copy(update=updates)
            
            selections_with_results.append(sr)
        
        # Vytvořit TicketResponse s selections co už mají results
        # Převeď created_at na ISO string (DB vrací datetime)
        created_at_str = None
        if row.get("created_at"):
            created_at_dt = row["created_at"]
            # Jestli je to datetime objekt, převeď na ISO string
        ...
        tr = TicketResponse(
            ...
            total_odds=float(row["ticket"].total_odds),
            combined_probability=row["ticket"].combined_probability,
            recommended_stake_pct=row["ticket"].recommended_stake_pct,
            selections=selections_with_results,
            ticket_id=row["ticket_id"],
            status=row["status"],
            live_alert=row["live_alert"],
            actual_stake_amount=row["actual_stake_amount"],
