...
def replace_selection(req: TicketGenerateRequest, exclude_match_ids: list[int] = [], user_id: int = Depends(get_current_user_id)):
    """Vygeneruje nový výběr místo vyhozeného — používá stejný pool zápasů, jen přeskočí vyloučené zápasy."""
    matches = _fetch_candidate_matches(req.sports, req.time_frame_days)
    # Filtruj vyloučené zápasy
    matches = [m for m in matches if m.match_id not in exclude_match_ids]
    result = ticket_generator.generate(
        matches, req.risk_level, req.sports, req.market_types, req.time_frame_days,
        pool_filter=ai_reviewer.review_candidates,
    )
