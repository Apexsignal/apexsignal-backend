...
def _filter_future_matches(matches: list[MatchInput], buffer_minutes: int = 5) -> list[MatchInput]:
    ...
    for m in matches:
        try:
            ...
            # FIX: Pokud je kickoff_time prázdný či špatný (API vrací chybný formát), použít default 12:00
            try:
                kickoff_time = m.kickoff_time if (m.kickoff_time and ':' in str(m.kickoff_time) and len(str(m.kickoff_time)) >= 5) else "12:00"
                kickoff_str = f"{m.kickoff_date}T{kickoff_time}:00Z"  # "2024-07-18T15:00:00Z"
                kickoff_dt = datetime.fromisoformat(kickoff_str.replace('Z', '+00:00'))
            except (ValueError, TypeError):
                # Fallback: Pokud parsing pořád selhá, použít default
                kickoff_dt = datetime.fromisoformat(f"{m.kickoff_date}T12:00:00+00:00")
