...
def _run_debug_today_full_job(request_id: str) -> None:
    try:
        ...
        def selection_dict(c):
            return {
                "home_team": c.home_team, "away_team": c.away_team, "league": c.league,
                "market_type": c.market_type.value, "selection": c.selection,
                "model_probability": round(c.model_probability, 3),
                "market_probability": round(c.market_probability, 3) if c.market_probability is not None else None,
                "probability": round(c.probability, 3), "odds": c.odds,
                "kickoff_date": c.kickoff_date, "kickoff_time": c.kickoff_time,
            }

        full_pool = ticket_generator._build_filtered_pool(
            today_matches, DAILY_TICKETS_SPORTS, DAILY_TICKETS_MARKETS, min_prob=0.65,
        )
        all_passing = [selection_dict(c) for c in full_pool]
        gc.collect()  # viz stejná pojistka v _run_generate_job (OOM na starter plánu)

        chat_id = os.environ.get("TELEGRAM_CHAT_ID")
        tickets_out = []
        exclude_ids: set[int] = set()
        remaining = today_matches
        for i in range(2):
            pool_matches = [m for m in remaining if m.match_id not in exclude_ids]
            result = ticket_generator.generate(pool_matches, 20, DAILY_TICKETS_SPORTS, DAILY_TICKETS_MARKETS, 3)
            ticket = result["safe"]
            if ticket is None:
                tickets_out.append({"ticket_number": i + 1, "status": "nepovedlo se sestavit"})
                continue
            for s in ticket.selections:
                exclude_ids.add(s.match_id)
            sent = False
            if chat_id:
                try:
                    ...
                    print(f"[_debug_today_full] Telegram send failed: {e}")
            tickets_out.append({
                "ticket_number": i + 1,
                "total_odds": ticket.total_odds,
                "combined_probability": round(ticket.combined_probability, 3),
        ...
        _results_store(request_id, {
            "status": "done",
            "today_date": today,
            "today_matches_count": len(today_matches),
            "tickets": tickets_out,
            "all_passing_candidates_count": len(all_passing),
            "all_passing_candidates": all_passing,
        })
    except Exception as e:
        _results_store(request_id, {"status": "error", "detail": str(e)})


@app.post("/admin/_debug-today-full")
def _debug_today_full(request: Request):
    admin_key_expected = os.environ.get("ADMIN_TASK_KEY")
    if not admin_key_expected or request.headers.get("X-Admin-Key") != admin_key_expected:
        raise HTTPException(status_code=403, detail="Neplatný nebo chybějící X-Admin-Key")

    request_id = secrets.token_urlsafe(8)
    _results_store(request_id, {"status": "running"})
    threading.Thread(target=_run_debug_today_full_job, args=(request_id,), daemon=True).start()
    return {"request_id": request_id}


@app.get("/admin/_debug-today-full-result")
def _debug_today_full_result(request: Request, request_id: str):
    admin_key_expected = os.environ.get("ADMIN_TASK_KEY")
    if not admin_key_expected or request.headers.get("X-Admin-Key") != admin_key_expected:
        raise HTTPException(status_code=403, detail="Neplatný nebo chybějící X-Admin-Key")
    payload = _results_get(request_id)
    if payload is None:
        raise HTTPException(status_code=404, detail="Neznámé nebo vypršelé request_id")
    return payload
