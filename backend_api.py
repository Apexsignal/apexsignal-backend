...
def _debug_generate_today(request: Request, email: str, risk_level: int = 20):
    """DOČASNÝ debug endpoint — spustí reálný ticket_generator.generate()
    pro dnešek na zadaný účet, ULOŽÍ, ale NEPOSÍLÁ na Telegram (jen appka
    chce vidět výslednou kombinaci). Po použití appka tenhle endpoint
    zase smaže."""
    admin_key_expected = os.environ.get("ADMIN_TASK_KEY")
    if not admin_key_expected or request.headers.get("X-Admin-Key") != admin_key_expected:
        raise HTTPException(status_code=403, detail="Neplatný nebo chybějící X-Admin-Key")
    ...
    return {
        "status": "saved", "ticket_id": ticket_id, "total_odds": ticket.total_odds,
        "selections": [{"match": f"{s.home_team} - {s.away_team}", "market": s.market_type.value, "selection": s.selection, "probability": s.probability, "odds": s.odds, "reasoning": s.reasoning} for s in ticket.selections],
    }
