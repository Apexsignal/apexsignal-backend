...
def _fetch_candidate_matches(sports: list[Sport], time_frame_days: int) -> list[MatchInput]:
    ...
    Filtruje jen ligy dostupné na Tipsportu!
    Data_provider.get_upcoming_matches() vrátí přesně zápasy v tomto rozsahu.
    """
    # Ligy dostupné na Tipsportu
    TIPSPORT_LEAGUES = {
        "czech republic",  # Česká Chance Liga
    ...
    for sport in sports:
        ...
        # Filtruj jen zápasy z lig dostupných na Tipsportu
        if sport == Sport.FOOTBALL:
            sport_matches = [
                m for m in sport_matches 
                if m.country.lower() in TIPSPORT_LEAGUES
            ]
