...
def _debug_cards_ht_calibration(request: Request, days_back: int = 5, cards_threshold: float = 3.5, ht_threshold: float = 0.5):
    ...
    for i in range(1, days_back + 1):
        ...
        raw_items = provider._get("/fixtures", {"date": day})
        ...
        for m in matches:
            ...
            # Karty — appka to bere přes SKUTEČNÝ tržní kurz (over_cards_odds),
            # stejně jako by to dělalo ostré generování, ne jen model naslepo.
            if m.over_cards_odds and cards_threshold in m.over_cards_odds:
                try:
                    result = data_provider.adapt_fixture_result(provider.get_fixture_result(str(m.match_id)))
                    if result["is_finished"]:
                        stats = provider.get_fixture_statistics(m.match_id)
                        total_cards = data_provider.adapt_fixture_card_count(stats)
                        if total_cards is not None:
                            model_prob = MarketEvaluator.over_cards_probability(m.expected_cards, cards_threshold)
                            cards_rows.append({
                                "match": f"{m.home_team} - {m.away_team}", "date": day,
                                "model_prob": model_prob, "odds": m.over_cards_odds[cards_threshold],
                                "actual_total_cards": total_cards,
                                "hit": total_cards > cards_threshold,
                            })
                except Exception as e:
                    cards_rows.append({"match": f"{m.home_team} - {m.away_team}", "date": day, "error": str(e)})

            # Poločasové góly — appka NEMÁ skutečný tržní kurz (nikde
            # nenaplněné ht_over_goals_odds), takže tohle je čistě appky
            # model proti realitě, bez ověření trhem.
            if m.home_expected_goals_ht and m.away_expected_goals_ht:
                try:
                    result = data_provider.adapt_fixture_result(provider.get_fixture_result(str(m.match_id)))
                    if result["is_finished"] and result["ht_home_goals"] is not None:
                        model_prob = MarketEvaluator.ht_over_goals_probability(
                            m.home_expected_goals_ht, m.away_expected_goals_ht, ht_threshold
                        )
                        actual_ht_total = result["ht_home_goals"] + result["ht_away_goals"]
                        ht_rows.append({
                            "match": f"{m.home_team} - {m.away_team}", "date": day,
                            "model_prob": model_prob, "actual_ht_goals": actual_ht_total,
                            "hit": actual_ht_total > ht_threshold,
                        })
                except Exception as e:
                    ht_rows.append({"match": f"{m.home_team} - {m.away_team}", "date": day, "error": str(e)})

    def _summarize(rows, min_prob):
        eligible = [r for r in rows if r.get("model_prob", 0) >= min_prob]
    ...
    return {
        "days_back": days_back,
        "cards_threshold": cards_threshold,
        "cards_calibration": {
            "vsechny_kandidaty_se_skut_kurzem": len(cards_rows),
            "nad_65pct": _summarize(cards_rows, 0.65),
            "nad_71pct": _summarize(cards_rows, 0.71),
            "detail": cards_rows,
        },
        "ht_threshold": ht_threshold,
        "ht_goals_calibration_MODEL_ONLY_no_real_odds": {
            "vsechny_kandidaty": len(ht_rows),
            "nad_65pct": _summarize(ht_rows, 0.65),
            "nad_71pct": _summarize(ht_rows, 0.71),
            "detail": ht_rows,
        },
    }
