...
def _filter_future_matches(matches: list[MatchInput], buffer_minutes: int = 5) -> list[MatchInput]:
    ...
    for m in matches:
        try:
            ...
            # OPRAVA: Pokud je kickoff_time prázdný nebo špatný, použít default 12:00
            kickoff_time = m.kickoff_time if m.kickoff_time and m.kickoff_time.strip() and ':' in m.kickoff_time else "12:00"
            kickoff_str = f"{m.kickoff_date}T{kickoff_time}:00Z"  # "2026-07-18T15:00:00Z" nebo "2026-07-18T12:00:00Z"
            try:
                kickoff_dt = datetime.fromisoformat(kickoff_str.replace('Z', '+00:00'))
            except (ValueError, TypeError):
                # API vrací špatný formát: '2026-07-21T:00+00:00' (chybí hodiny:minuty)
                # Fallback: doplnit výchozí čas 12:00
                try:
                    date_part = kickoff_str.split('T')[0]
                    kickoff_dt = datetime.fromisoformat(f"{date_part}T12:00:00+00:00")
                except:
                    continue  # Přeskočit zápas s zcela špatným formátem
            
            # Vezmi všechny zápasy bez ohledu na čas
            # (Uživatelova odpovědnost vygenerovat ráno, ne večer)
...
def check_duplicate_matches(req: dict = Body(...), user_id: int = Depends(get_current_user_id)):
    ...
    for selection in req.get("selections", []):
        # Handle both 'match_id' and 'id' field names
        match_id = selection.get("match_id") or selection.get("id")
        if not match_id:
            continue
            
        if match_id in pending_ids_set:
            # Najdi detaily z pending matches
            for pm in pending_matches:
                if pm["match_id"] == match_id:
                    duplicates.append({
