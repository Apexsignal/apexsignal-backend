...
@app.get("/admin/_debug-generate-full-again")
def _debug_generate_full_again(request: Request, time_frame_days: int = 1):
    """Dočasné: appka sestaví tiket ze VŠECH trhů (bez vylučování
    uživatelovy historie) — ověření, jestli appka upřednostní Dvojtip
    před appky Nad góly, jak appka slibuje MARKET_PRIORITY."""
    admin_key_expected = os.environ.get("ADMIN_TASK_KEY")
    if not admin_key_expected or request.headers.get("X-Admin-Key") != admin_key_expected:
        raise HTTPException(status_code=403, detail="Neplatný nebo chybějící X-Admin-Key")
