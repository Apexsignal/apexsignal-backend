...
@app.post("/admin/_debug-generate-including-saved")
def admin_debug_generate_including_saved(email: str, request: Request):
    """
    Jednorázový admin bypass (2026-09-09, na výslovnou žádost uživatele) —
    normální /tickets/generate vylučuje zápasy z JAKÉHOKOLIV uloženého
    tiketu (i pending), tenhle debug endpoint tu výjimku vynechá jen pro
    tohle jedno volání, ať appka může nabídnout i zápasy, co uživatel
    má rozpracované v jiném pending tiketu. Neukládá se natrvalo do
    generační logiky, jen jedno ruční spuštění.
    """
    admin_key_expected = os.environ.get("ADMIN_TASK_KEY")
    if not admin_key_expected or request.headers.get("X-Admin-Key") != admin_key_expected:
        raise HTTPException(status_code=403, detail="Neplatný nebo chybějící X-Admin-Key")
    ...
    exclude_ids = set(repo.get_last_batch(user_id))  # bez all_saved_match_ids
    ...
    return {
        ...
        "status": "saved",
        "selections": [
            {"home_team": s.home_team, "away_team": s.away_team, "market_type": s.market_type.value,
