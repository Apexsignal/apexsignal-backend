...
def admin_debug_seed_referral_earning(request: Request):
    ...
    db.record_referral_membership_earning(
        referrer["id"], referred["id"], "cs_test_ico_preview_1", "monthly", 9900, 0.50, 4950,
    )
...
def admin_debug_cleanup_referral_earning(request: Request):
    ...
    if not admin_key_expected or request.headers.get("X-Admin-Key") != admin_key_expected:
        raise HTTPException(status_code=403, detail="Neplatný nebo chybějící X-Admin-Key")
    with db.get_cursor() as cur:
        cur.execute("DELETE FROM referral_membership_earnings WHERE stripe_ref = %s RETURNING id", ("cs_test_ico_preview_1",))
        deleted = cur.rowcount
        cur.execute("DELETE FROM referral_payout_requests WHERE account_number = %s RETURNING id", ("TEST_ICO_PREVIEW",))
