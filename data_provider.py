...
class APIFootballProvider(SportsDataProvider):
    ...
    def get_upcoming_matches(self, sport, days_ahead):
        ...
        # Načti filtr lig z env proměnné — pokud není nastavena, stahuj vše
        ...
        if league_ids_raw:
            try:
                ...
                print(f"[APIFootball] filtr lig aktivní: {sorted(allowed_league_ids)}")
            except ValueError:
                print("[APIFootball] FOOTBALL_LEAGUE_IDS má špatný formát, ignoruji filtr")

        fixtures = []
        today = date.today()

        if allowed_league_ids:
            # Stahuj per liga per den — přesnější, ale víc requestů
            season = _season_year_for_league(39, today)  # EPL jako referenční sezóna
            per_league_limit = max(MAX_FIXTURES_PER_REQUEST // max(len(allowed_league_ids), 1), 2)
            for league_id in sorted(allowed_league_ids):
                for offset in range(days_ahead + 1):
                    day = today + timedelta(days=offset)
                    try:
                        day_fixtures = self._get("/fixtures", {
                            "league": league_id,
                            "season": _season_year_for_league(league_id, today),
                            "date": day.isoformat(),
                        })
                        fixtures.extend(day_fixtures[:per_league_limit])
                        time.sleep(0.25)
                    except Exception as e:
                        print(f"[APIFootball] liga {league_id} den {day}: {e}")
                        continue
                if len(fixtures) >= MAX_FIXTURES_PER_REQUEST:
                    break
        else:
            # Bez filtru — stahuj všechny zápasy daného dne
        ...
        print(f"[APIFootball] celkem {len(fixtures)} zápasů po filtrování")
        self._cache.set(cache_key, fixtures)
        return fixtures
