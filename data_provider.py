...
# denní kvótu API na jediné generování (appka na 1 zápas potřebuje cca
# 5-7 dalších volání — statistiky, kurzy, forma, zranění, tabulka).
MAX_FIXTURES_PER_REQUEST = 60
...
class APIFootballProvider(SportsDataProvider):
    ...
    def get_upcoming_matches(self, sport: Sport, days_ahead: int) -> list[dict]:
        """
        Appka stáhne VŠECHNY zápasy (bez ohledu na ligu/soutěž), co se hrají
        ve zvoleném okně dní — ne jen z předem vybraného seznamu lig. Appka
        pro každý den v okně zavolá /fixtures?date=YYYY-MM-DD, což vrátí
        kompletní denní program přes všechny soutěže, co API-Football pokrývá
        (1200+ soutěží).

        POZOR na rozpočet API: appka pro KAŽDÝ zápas dál dělá dalších cca
        5-7 volání (statistiky obou týmů, kurzy, forma, zranění, tabulka).
        U dne s desítkami soutěží by appka snadno vyčerpala celou denní
        kvótu na jediné generování — proto appka výsledek tvrdě omezí na
        MAX_FIXTURES_PER_REQUEST (řadí podle data/hodiny výkopu, ne podle
        důležitosti soutěže — appka nemá spolehlivý způsob, jak soutěže
        řadit podle "kvality").
        """
    ...
    def get_team_statistics(self, sport: Sport, team_id: str, league_id: Optional[str] = None) -> dict:
        ...
        if cached is not None:
            return cached
        # API-Football vyžaduje 'league' parametr — appka to ověřila naživo
        # (bez league appka dostane zpátky chybu "The League field is
        # required."). Bez league_id appka nemá smysl volat dál — vrátí
        # prázdné statistiky a appka spadne zpátky na ligový průměr (shrinkage).
        if not league_id:
            return {}
