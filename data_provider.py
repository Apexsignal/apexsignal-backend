...
# ===== PROVIDER FACTORY =====
class APIFootballV3Provider(SportsDataProvider):
    """Speciální provider pro Football API v3"""
    def __init__(self, api_key: Optional[str] = None, cache_ttl_seconds: int = 300):
        self.api_key = api_key or os.environ.get("APISPORTS_KEY", "")
        if not self.api_key:
    ...
    def get_upcoming_matches(self, sport: Sport, days_ahead: int) -> list[dict]:
        ...
        if cached is not None:
            return cached
        fixtures = self._get("/fixtures", {"status": "NS", "timezone": "Europe/Prague", "next": 300})
        self._cache.set(cache_key, fixtures)
        return fixtures
    ...
    def get_team_statistics(self, sport: Sport, team_id: str, league_id: Optional[str] = None) -> dict:
        ...
        if cached is not None:
            return cached
        stats = self._get(f"/teams/statistics", {"team": team_id, "season": 2025})
    ...
    def get_pre_match_odds(self, match_id: str) -> dict:
        ...
        if cached is not None:
            return cached
        odds = self._get("/odds", {"fixture": match_id})
        self._cache.set(cache_key, odds[0] if odds else {})
        return odds[0] if odds else {}
    
    def get_recent_form(self, team_id: str, last: int = 10) -> list[dict]:
        return self._get(f"/fixtures", {"team": team_id, "last": last})
    
    def get_injuries(self, match_id: str) -> list[dict]:
        return self._get(f"/injuries", {"fixture": match_id})
    
    def get_standings(self, league_id: str, season: Optional[str] = None) -> list[dict]:
...
def get_provider(sport: Sport):
    ...
    if sport == Sport.FOOTBALL:
        if not api_key:
            raise RuntimeError("Chybí APISPORTS_KEY!")
        return APIFootballV3Provider(api_key)
