...
class APIFootballProvider(SportsDataProvider):
    ...
    def get_upcoming_matches(self, sport: Sport, days_ahead: int) -> list[dict]:
        ...
        for offset in range(days_ahead + 1):
            ...
            # Filtruj jen ligy dostupné na Tipsport
            day_fixtures = [
                f for f in day_fixtures
                if f.get("league", {}).get("id") in TIPSPORT_LEAGUE_IDS
            ]
            # Vyřaď zápasy co už začaly nebo skončily
            def is_upcoming(f: dict) -> bool:
                ...
                if kickoff:
                    try:
                        ...
                        ko_utc = ko.astimezone(timezone.utc).replace(tzinfo=None)
                        # Přidej 30 minut buffer — výběr na zápas co začíná za méně než 30 min nedává smysl
