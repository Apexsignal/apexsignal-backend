...
class APISportsDirectProvider(SportsDataProvider):
    def __init__(self, sport_path: str, api_key: Optional[str] = None, cache_ttl_seconds: int = 300):
        self.sport_path = sport_path  # "basketball" nebo "hockey" nebo "football"
        ...
        # FOOTBALL POUŽÍVÁ V3, OSTATNÍ V1!
    ...
    def get_upcoming_matches(self, sport: Sport, days_ahead: int) -> list[dict]:
        ...
        # FOOTBALL používá V3 s /matches
        if self.sport_path == "football":
            from datetime import timedelta
            today = date.today()
            # Volej /matches pro každý den zvlášť
            for offset in range(days_ahead + 1):
                day = today + timedelta(days=offset)
                try:
                    day_games = self._get("/matches", {
                        "date": day.isoformat(),
                        "status": "NS",
                    })
                    ...
                    print(f"Warning: nemůžu stáhnout zápasy pro {day}: {e}")
                    pass
        else:
            # OSTATNÍ sporty používají /games
            today = date.today()
            from datetime import timedelta
            for offset in range(days_ahead + 1):
                day = today + timedelta(days=offset)
...
def get_provider(sport: Sport):
    ...
    if sport == Sport.FOOTBALL:
        ...
        return APISportsDirectProvider("football", api_key)
    
    elif sport == Sport.HOCKEY:
        if not api_key:
            raise RuntimeError("Chybí APISPORTS_KEY!")
        return APISportsDirectProvider("hockey", api_key)
    
    elif sport == Sport.BASKETBALL:
        if not api_key:
            raise RuntimeError("Chybí APISPORTS_KEY!")
        return APISportsDirectProvider("basketball", api_key)
...
# ===== ADAPT FUNKCE PRO API-FOOTBALL V3 =====

def adapt_api_football_fixture(raw: dict) -> dict:
    """Mapuj raw data z /matches na fixture strukturu"""
    return {
        "id": raw.get("fixture", {}).get("id"),
        "league_id": raw.get("league", {}).get("id"),
        "home_team_id": raw.get("teams", {}).get("home", {}).get("id"),
        "away_team_id": raw.get("teams", {}).get("away", {}).get("id"),
        "home_team": raw.get("teams", {}).get("home", {}).get("name", "Home"),
        "away_team": raw.get("teams", {}).get("away", {}).get("name", "Away"),
        "date": raw.get("fixture", {}).get("date"),
        "status": raw.get("fixture", {}).get("status", {}).get("short"),
    }


def adapt_api_football_team_stats(stats: dict) -> dict:
    """Mapuj team statistics na standardní strukturu"""
    if not stats:
        return {
            "games_played": 1,
            "avg_goals_scored_last_10": 1.2,
            "avg_goals_conceded_last_10": 1.2,
        }
    
    return {
        "games_played": stats.get("games_played", 1),
        "avg_goals_scored_last_10": stats.get("avg_goals_scored_last_10", 1.2),
        "avg_goals_conceded_last_10": stats.get("avg_goals_conceded_last_10", 1.2),
    }


def adapt_api_football_odds(odds: dict) -> dict:
    """Mapuj odds z API na strukturu"""
    if not odds:
        return {}
    ...
    for bookmaker in bookmakers:
        for bet in bookmaker.get("bets", []):
            ...
            for odd in bet.get("values", []):
                key = odd.get("value")
                value = float(odd.get("odd", 1.0))
                bets[bet_type][key] = value
    
    return bets


def adapt_api_football_live_stats(minute: int, raw: dict) -> dict:
    """Mapuj live statistics"""
    return {
        "minute": minute,
        "home_goals": raw.get("goals", {}).get("home", 0),
        "away_goals": raw.get("goals", {}).get("away", 0),
    }
