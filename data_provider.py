...
Účel:
    ...
    Pozn.: Reálný API klíč se dosazuje přes proměnnou prostředí
    RAPIDAPI_FOOTBALL_KEY (nastav ji na serveru, kde poběží backend —
...
# =======================================================================
# API-FOOTBALL (RapidAPI) — konkrétní implementace
...
class APIFootballProvider(SportsDataProvider):
    def __init__(self, rapidapi_key: Optional[str] = None, cache_ttl_seconds: int = 30):
        self.api_key = rapidapi_key or os.environ.get("RAPIDAPI_FOOTBALL_KEY", "")
        if not self.api_key:
            raise RuntimeError(
                "Chybí RAPIDAPI_FOOTBALL_KEY. Nastav ho jako proměnnou prostředí na "
                "serveru, kde běží backend (Render/Railway/Fly.io) — NIKDY ho nepiš "
                "přímo do kódu ani do žádného souboru, který skončí ve frontendu."
            )
        self._cache = InMemoryCache(ttl_seconds=cache_ttl_seconds)

    def _headers(self) -> dict:
        return {"x-rapidapi-key": self.api_key, "x-rapidapi-host": API_FOOTBALL_HOST}
