...
class TicketGenerator:
    ...
    def _build_ticket(
        ...
        MIN_PROBABILITY = {
            "kratky": 0.54,   # KRÁTKÝ: 54% min (baseline, tvůj úspěšný track record!)
            "stredni": 0.58,  # STŘEDNÍ: 58% min (trochu vyšší pro bezpečnost)
            "boost": 0.60,    # BOOST: 60% min (jen málo matchů, kurz 10-15 je bonus)
        }
        min_selections = MIN_SELECTIONS.get(ticket_type, 2)
        min_odds_hard = MIN_ODDS_HARD.get(ticket_type, 2.0)
        min_probability = MIN_PROBABILITY.get(ticket_type, 0.55)

        # Kontrola: všechny vybrané zápasy musí být nad minimem
        if any(c.probability < min_probability for c in selected):
            return None
