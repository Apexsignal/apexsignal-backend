...
def get_over_threshold_for_league(league: str) -> float:
    """
    Dynamický Over threshold podle ligy!
    
    Top ligy (Premier, La Liga, Bundesliga, Serie A, Ligue 1) = Over 1.5+
    Ostatní ligy = Over 1.5
    """
    league_lower = league.lower() if league else ""
    
    # Všechny ligy = Over 1.5
    return 1.5


MIN_SELECTION_PROBABILITY = 0.70  # Zvýšeno na 70%! - konzervativní + odds calibration
MIN_SELECTION_PROBABILITY_AGGR = 0.70  # stejný pro všechny delší tikety
ODDS_CALIBRATION_WEIGHT = 0.6  # 60% důvěry v odds (Tipsport expert), 40% v model
MIN_EDGE_FOR_SELECTION = 0.02  # Minimální edge: 2% pro inclusion
...
KELLY_FRACTION = 0.35         # Zvýšeno na 35% (bylo 25%) - více agresivní! Model teď lepší s odds calibration
...
class MarketEvaluator:
    ...
    @staticmethod
    def calibrate_by_odds(model_probability: float, market_odds: Optional[float]) -> float:
        """
        Kalibruje model pravděpodobnost podle Tipsport odds.
        Tipsport = expert (miliony eur v play!) → věříme jim 60%.
        
        Implied prob = 1 / odds
        Final prob = 60% * odds_prob + 40% * model_prob
        """
        if market_odds is None or market_odds <= 1.0:
            return model_probability
        
        # Implied probability z odds
        implied_prob = 1.0 / market_odds
        
        # Kalibrace: 60% Tipsport, 40% model
        calibrated = (ODDS_CALIBRATION_WEIGHT * implied_prob + 
                      (1 - ODDS_CALIBRATION_WEIGHT) * model_probability)
        
        return min(0.99, max(0.01, calibrated))  # Clamp: [0.01, 0.99]
    ...
    def build_candidates(cls, match: MatchInput, min_prob: float = MIN_SELECTION_PROBABILITY) -> list[SelectionCandidate]:
        ...
        if match.sport in (Sport.FOOTBALL, Sport.HOCKEY):
            ...
            # FILTRY:
            # 1. Nereálné xG - skip
            if match.home_expected_goals < 0.5 or match.away_expected_goals < 0.5:
                pass  # Skip - příliš nízké
            # 2. Forma - skip pokud home tým dal < 1.5 nebo away < 1.5 v posledních zápasech
            elif (match.home_expected_goals < 1.5 or match.away_expected_goals < 1.5):
                pass  # Skip - slabá forma
            # 3. Quality match - jen pokud je suma > 2.0
            elif (match.home_expected_goals + match.away_expected_goals) > 2.0:
                # MARGIN OF SAFETY: jen Over pokud je model VÝRAZNĚ nad threshold
                for threshold, odds in match.over_goals_odds.items():
                    ...
                    # MARGIN OF SAFETY: jen pokud je expected > threshold + 0.15
                    expected_total = match.home_expected_goals + match.away_expected_goals
                    if expected_total > (threshold + 0.15):  # Bezpečnostní rezerva!
        ...
        # Filtr: probability > min_prob NEBO (edge > 3% AND probability > min_prob - 0.05)
        # = Aggressive mode: nižší probability je OK pokud je edge vysoký!
        def passes_probability_filter(c: SelectionCandidate) -> bool:
            if c.probability > min_prob:
                return True
            # Aggressive: pokud je edge víc než 3%, vezmi i s nižší probability
            if c.edge and c.edge >= 0.03 and c.probability > min_prob - 0.05:
                return True
            return False

        return [c for c in candidates if passes_probability_filter(c) and passes_odds_filter(c)]
    ...
    def _candidate(match: MatchInput, market_type: MarketType, selection: str, probability: float, odds: float) -> SelectionCandidate:
        # model_probability je VŽDY náš vlastní heuristický odhad
        model_probability = probability
        market_key = f"{market_type.value}:{selection}"
        
        # ODDS CALIBRATION: Aplikuj Tipsport odds na model
        # Tipsport = expert (60% důvěry), Model = 40% důvěry
        final_probability = MarketEvaluator.calibrate_by_odds(model_probability, odds)
        
        market_probability = match.market_implied_probabilities.get(market_key)
        reasoning = MarketEvaluator._build_reasoning(match, market_type, selection, model_probability, final_probability)
        ...
        return SelectionCandidate(
            ...
            model_probability=model_probability, market_probability=final_probability,
...
class TicketGenerator:
    ...
    def generate(
        ...
        # Fallback — širší rozsah kurzu ale stále rozumný
        if ticket is None and pool:
            wider_range = (odds_range[0] * 0.75, odds_range[1] * 1.5)
        ...
        if ticket is None:
            # Poslední pokus — nižší práh pravděpodobnosti ale minimální kurz zachovaný
            min_acceptable_odds = odds_range[0] * 0.6  # max 40% pod dolní hranicí
            for threshold in [0.60, 0.55]:
                wider_pool = self._build_filtered_pool(matches, allowed_sports, allowed_markets, min_prob=threshold)
                if wider_pool:
                    fallback_range = (min_acceptable_odds, odds_range[1] * 2.0)
                    ticket = self._build_ticket(wider_pool, fallback_range, ticket_key, risk_level)
                    if ticket:
                        print(f"[TicketGenerator] Fallback tiket sestaven při prahu {threshold:.0%}, kurz {ticket.total_odds:.2f}")
                        break

        # Pokud ani fallback nedosáhl rozumného kurzu, raději vraťme None
        if ticket and ticket.total_odds < odds_range[0] * 0.5:
            print(f"[TicketGenerator] Fallback kurz {ticket.total_odds:.2f} je příliš nízký — vracím None")
            ticket = None
    ...
    def _build_filtered_pool(
        ...
        # EDGE FILTER: Jen selectiony s POZITIVNÍM edge
        # Model probability > Market probability = PROFIT!
        MIN_EDGE = 0.02  # Minimální edge: 2%
        pool = [c for c in pool if c.edge is not None and c.edge >= MIN_EDGE]
        
        # Řaď podle edge (nejvetší profit first!)
        pool.sort(key=lambda c: (c.edge or -999), reverse=True)
    ...
    def _build_ticket(
        ...
        # Minimální počet výběrů podle typu tiketu
        MIN_SELECTIONS = {"krátký": 2, "střední": 3, "dlouhý": 6}
        ...
        for candidate in ordered_pool:
            ...
            # ← ZMĚNA: zastavit JEN pokud máme MINIMÁLNÍ počet výběrů A je v rozsahu!
            if len(selected) >= min_selections and min_odds <= running_odds <= max_odds:
                break

        if not selected:
            print(f"[TicketGenerator] {ticket_type}: žádní kandidáti")
            return None

        # Pokud nedosáhneme cílového rozsahu — vrátíme co máme s poznámkou
        if not (min_odds <= running_odds <= max_odds):
            print(f"[TicketGenerator] {ticket_type}: částečný tiket (kurz {round(running_odds,2)}, cíl {min_odds}-{max_odds})")

        is_partial = not (min_odds <= running_odds <= max_odds)
        ...
        partial_note = f" ⚠️ Model našel jen {len(selected)} výběr{'y' if len(selected) > 1 else ''} s dostatečnou jistotou (kurz {round(running_odds,2)}, cíl {min_odds}–{max_odds}). Chceš přidat výběry s nižší pravděpodobností? Klikni 🔄." if is_partial else ""
