...
class MarketEvaluator:
    ...
    def build_candidates(cls, match: MatchInput) -> list[SelectionCandidate]:
        ...
        return [c for c in candidates if c.probability > MIN_SELECTION_PROBABILITY and c.odds >= MIN_SELECTION_ODDS]
...
class TicketGenerator:
    ...
    def generate(
        ...
        print(f"[TicketGenerator] {len(matches)} zápasů na vstupu, {len(pool)} kandidátů prošlo přes 70% pravděpodobnost + kurz >= {MIN_SELECTION_ODDS}")
        for c in pool[:5]:
            print(f"[TicketGenerator]   {c.home_team} vs {c.away_team}: {c.market_type.value} {c.selection} @ {c.odds} (p={round(c.probability,3)})")

        # Volitelný vnější filtr (např. AI kontrola čerstvých zpráv) — modul
        # samotný o ničem takovém neví, jen přijme hotovou funkci a použije ji.
        if pool_filter is not None:
            pool = pool_filter(pool)
            print(f"[TicketGenerator] po AI kontrole zůstalo {len(pool)} kandidátů")
