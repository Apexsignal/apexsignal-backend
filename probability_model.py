...
MIN_SELECTION_ODDS = 1.3  # appka odmítne výběry s kurzem pod touhle hranicí — i kdyby
                           # klidně prošly přes 70% pravděpodobnost, jsou skoro
                           # informačně prázdné (kurz 1,01 na "víc než 0,5 gólu" v
                           # neznámé nižší lize) a appka by jich do kombinace musela
                           # nacpat desítky, jen aby se dostala na cílový kurz
...
TICKET_RANGES = {
    "kratky":  (2.0,  3.5),   # Krátký — 2-3 výběry
    "stredni": (3.5,  6.0),   # Střední — 3-5 výběrů
    "dlouhy":  (6.0, 12.0),   # Dlouhý — 5-8 výběrů
}
...
class MarketEvaluator:
    ...
    def build_candidates(cls, match: MatchInput, min_prob: float = MIN_SELECTION_PROBABILITY) -> list[SelectionCandidate]:
        ...
        # striktní podmínka ze zadání: pravděpodobnost > 70 % A kurz >= 1,3 —
        # appka odmítne i "jisté" výběry, co nesou skoro žádnou informační
        # hodnotu (kurz 1,01 na víc než 0,5 gólu v neznámé nižší lize).
        return [c for c in candidates if c.probability > min_prob and c.odds >= MIN_SELECTION_ODDS]
...
class TicketGenerator:
    ...
    def generate(
        ...
        pool = self._build_filtered_pool(matches, allowed_sports, allowed_markets, min_prob=min_prob)
        print(f"[TicketGenerator] {len(matches)} zápasů — {ticket_key} pool: {len(pool)} kandidátů, cíl kurz {odds_range}")
        for c in pool[:5]:
        ...
        # Vracíme pod klíčem "safe" pro zpětnou kompatibilitu s frontendem
