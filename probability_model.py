...
TICKET_RANGES = {
    ...
    "boost":   (10.0, 15.0),   # BOOST — TOP výběry, kurz 10-15, 4-6 zápasů
}
...
class TicketGenerator:
    ...
    def generate(
        ...
        """Vracet 2 tikety: safe (krátký) + aggressive (střední)"""
        
        # VŽDY chceme vrátit 2 tikety - krátký i střední
        tickets = {}
        
        for ticket_key in ["kratky", "stredni"]:
            odds_range = TICKET_RANGES[ticket_key]
            min_prob = 0.70 if ticket_key == "kratky" else 0.70
            ...
            if used_threshold < min_prob:
                print(f"[TicketGenerator] Použit nižší práh {used_threshold:.0%} — nedostatek kandidátů")
            ...
            # Fallback - rozšířit rozsah kurzu
            ...
            # Poslední pokus - jakýkoliv rozsah
            if ticket is None:
            ...
            # Vrátit tiket jako safe nebo aggressive
            if ticket_key == "kratky":
                tickets["safe"] = ticket
            else:  # stredni
                tickets["aggressive"] = ticket
        
        # BOOST jen pokud risk_level > 60
        if risk_level > 60:
            ticket_key = "boost"
            odds_range = TICKET_RANGES[ticket_key]
            min_prob = 0.55
