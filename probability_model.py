...
class TicketGenerator:
    ...
    def generate(
        ...
        if used_threshold < min_prob:
            print(f"[TicketGenerator] Použit nižší práh {used_threshold:.0%}")
    ...
    def _build_ticket(
        ...
        MIN_ODDS_HARD = {
            "kratky": 1.8,  # Nikdy ne nižší
            "stredni": 1.9, # Nikdy ne nižší
            "boost": 9.0,   # Nikdy ne nižší
        }
        min_selections = MIN_SELECTIONS.get(ticket_type, 2)
        min_odds_hard = MIN_ODDS_HARD.get(ticket_type, 1.5)

        if len(selected) < min_selections or running_odds < min_odds_hard or not (min_odds <= running_odds <= max_odds):
            print(f"[TicketGenerator] {ticket_type}: NOT OK (výběry {len(selected)}/{min_selections}, kurz {round(running_odds,2)} min:{min_odds_hard}, cíl {min_odds}-{max_odds})")
            return None
