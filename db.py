...
def _migrate_add_boost_constraint():
    """Opravit CHECK constraint aby povoloval 'boost' ticket_type."""
    try:
        with get_cursor() as cur:
            # Smazat starý constraint pokud existuje
            cur.execute("""
                ALTER TABLE tickets 
                DROP CONSTRAINT IF EXISTS tickets_ticket_type_check;
            """)
            # Přidat nový constraint s 'boost'
            cur.execute("""
                ALTER TABLE tickets 
        ...
        print(f"[db.py] Migration warning (constraint možná již existuje): {e}")

# Spustit migraci při importu
_migrate_add_boost_constraint()
