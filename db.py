    ...
    with get_cursor() as cur:
        ...
        cur.execute("""
            ...
            BEGIN
                ...
                UPDATE ticket_selections SET result = 'pending' WHERE result IS NULL;
...
def _row_to_dict(ticket_row: dict, selection_rows: list[dict]) -> dict:
    ...
    return {
        ...
        "created_at": ticket_row.get("created_at"),  # Bezpečně - když neexistuje = None
    }
...
def fetch_ticket_rows(user_id: Optional[int] = None, status: Optional[str] = None) -> list[dict]:
    ...
    with get_cursor() as cur:
        ...
        for trow in ticket_rows:
            ...
            # Přidej raw selections se skutečnými DB id — potřebuje je settlement
            row_dict["selections"] = [
                {
                    "id": sr["id"],
...
def update_ticket_status(ticket_id: int, status: str) -> None:
    """Update ticket status: pending -> won/lost"""
    with get_cursor() as cur:
...
def delete_selection(ticket_id: int, selection_index: int) -> None:
    """Smaže selection ze tiketu podle indexu"""
    with get_cursor() as cur:
        # Najdi všechny selectiony pro tiket
        cur.execute("SELECT id FROM ticket_selections WHERE ticket_id = %s ORDER BY id", (ticket_id,))
        selections = cur.fetchall()
        if selection_index < 0 or selection_index >= len(selections):
            raise ValueError(f"Selection index {selection_index} out of range")
        # Smaž konkrétní selection
        selection_id = selections[selection_index]["id"]
